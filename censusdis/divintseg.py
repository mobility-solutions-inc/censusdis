...
This module was incorporated from the standalone ``censusdis/divintseg``
package so censusdis can support pandas 3 without depending on a release of
that package with a pandas 2 upper bound.

Source: https://github.com/censusdis/divintseg
"""
...
def diversity(
    ...
    if isinstance(communities, pd.DataFrame):
        ...
        return _diversity_of_df(pd.DataFrame([communities]))[0].iloc[0]
...
def _drop_non_numeric_except(
    ...
    """
    Drop non-numeric columns except grouping columns.

    Non-numeric columns may be present, e.g. for different political
    boundaries that are not needed for the current diversity or integration
    calculation.
    ...
    drop_cols = [
        ...
        if (not pd.api.types.is_numeric_dtype(df[col]) and col != by and col != over)
    ]
...
def integration(
    ...
    *,
    ...
    """
    Compute integration for one or more communities.

    Compute over a nested level of population aggregation. For example, with
    US census data we might compute integration of block groups over blocks.
    ...
    def integration_of_group(df_group: pd.DataFrame) -> float:
        """Compute the integration of one group."""
        if over is not None:
...
def segregation(
    ...
    *,
    ...
    """
    Compute segregation for one or more communities.

    Compute over a nested level of population aggregation. For example, with
    US census data we might compute segregation of block groups over blocks.
...
def di(
    ...
    *,
    ...
    """
    Compute diversity, integration, and optionally segregation.

    Compute these metrics for each of a collection of communities.
...
class SimilarityReference:
    ...
    def dissimilarity(
        ...
        Compare each community relative to a reference community.
...
def likelihood_populationfrac_product(
    ...
    """
    Multiply a likelihood series by a population fraction.

    This helper is shared by the isolation and exposure metrics.
...
def isolation(
    ...
    """
    Compute the isolation of a group in a community.

    Isolation is the average, over all members of a group in a community, of
    the proportion of the smaller area they reside in that are not members of
    their group.
    ...
    >>> from censusdis.divintseg import isolation
    ...
...
def bells(
    ...
    """
    Compute isolation using the index by Wendell Bell.
...
def exposure(
    ...
    """
    Compute the exposure of a group in a community.

    Exposure measures a group's average local exposure to members of another
    group.
    ...
    >>> from censusdis.divintseg import exposure
    ...
